...
class Solution
{
    ...
    void getNodeList(Node node, ArrayList<Integer> list){
        if(node==null) return;
    ...
    Node solve(ArrayList<Integer> list, int lo, int hi){
        if(lo > hi) return null;
        
        int mid = lo + (hi - lo)/2;
        Node node = new Node(list.get(mid));
        node.left = solve(list, lo, mid-1);
        node.right = solve(list, mid+1, hi);
        return node;
    }
}
