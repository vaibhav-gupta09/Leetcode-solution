...
class Solution{
    ...
    static int helper(Node node, int home, int k){
        ...
        int left = helper(node.left, home, k);
        if(left<k && left>=0){
            sum += node.data;
            calcSum(node.right, left-1);
            return left-1;
        }
        
        int right = helper(node.right, home, k);
        if(right < k && right>=0){
            sum += node.data;
            calcSum(node.left, right-1);
            return right-1;
        }
    ...
    static void calcSum(Node node, int k){
        if(k<0 || node==null) return;
        
        sum += node.data;
        calcSum(node.left, k-1);
        calcSum(node.right, k-1);
    }
}
